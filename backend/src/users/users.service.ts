...
export class UsersService {
  ...
  async createWithRoleValidation(
    ...
    if (creatorRole === 'HOSPITAL_ADMIN') {
      ...
      } else if (['SUPER_ADMIN', 'WOREDA_ADMIN', 'SYSTEM_ADMIN'].includes(newRole)) {
        throw new BadRequestException('Hospital Admin cannot create Super Admins, System Admins, or Woreda Admins');
      }
    } else if (creatorRole === 'SYSTEM_ADMIN') {
      if (!creatorAssignedRegion) {
        throw new BadRequestException('System Admin must have an assigned region to create users');
      }

      if (['SUPER_ADMIN', 'SYSTEM_ADMIN'].includes(newRole)) {
        throw new BadRequestException('System Admin cannot create Super Admins or System Admins');
      }

      if (newRole === 'WOREDA_ADMIN') {
        if (!woredaId) {
          throw new BadRequestException('Woreda Admin requires a woredaId');
        }
        await this.ensureWoredaMatchesRegion(woredaId, creatorAssignedRegion);
      }

      if (['HOSPITAL_ADMIN', 'DOCTOR', 'NURSE', 'MIDWIFE', 'DISPATCHER', 'LIAISON_OFFICER', 'HOSPITAL_APPROVER', 'GATEKEEPER', 'SPECIALIST'].includes(newRole)) {
        if (!hospitalId) {
          throw new BadRequestException(`${newRole} requires a hospitalId`);
        }
        await this.ensureHospitalMatchesRegion(hospitalId, creatorAssignedRegion);
      }
